...
/*   Updated: 2023/08/28 01:30:31 by lhasmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	process_words(struct s_word *w, char ***argv, int **args,
		t_shell *g_shell)
{
	char	*arg;
...
void	parse_ast(t_shell *g_shell, t_node *node, int **args, char ***argv)
{
	struct s_word	*w;
	t_node			*child;
	...
	while (child)
	{
		...
		if (!w)
			continue ;
		if (w)
			process_words(w, argv, args, g_shell);
		child = child->next_sibling;
	}
	if (check_buffer_bounds(g_shell, args[0], args[1], argv))
		(*argv)[(*args[0])] = NULL;
